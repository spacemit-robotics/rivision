...
void YOLOPipeline::preproc_worker() {
    while (running_) {
        ...
        
        {
            ...
            preproc_cv_.wait(lock, [this]() {
                return !preproc_queue_.empty() || !running_;
            });
    ...
    while (running_) {
        ...
        
        {
            ...
            infer_cv_.wait(lock, [this]() {
                return !infer_queue_.empty() || !running_;
            });
        ...
        if (!data->valid) {
            ...
            // 直接调用底层推理 (跳过预处理)
            result = detector_->detect_from_tensor(
                data->tensor, data->orig_width, data->orig_height);
            
            auto end = std::chrono::high_resolution_clock::now();
            result.inference_ms = std::chrono::duration_cast<
...
std::future<DetectionResult> YOLOPipeline::detect_async(
    ...
    
    {
        std::unique_lock<std::mutex> lock(preproc_mutex_);
        // 等待队列有空间
        preproc_space_cv_.wait(lock, [this]() {
            return preproc_queue_.size() < static_cast<size_t>(max_queue_size_) || !running_;
        });
...
DetectionResult YOLOPipeline::detect(const std::vector<uint8_t>& jpeg_data) {
    return detect_async(jpeg_data).get();
}
