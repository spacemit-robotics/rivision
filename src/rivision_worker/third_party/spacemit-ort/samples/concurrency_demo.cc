...
int main(int argc, char ** argv) {
    if (argc < 3) {
        fprintf(stderr,
                ...
                "[img_file_path=str] [num_threads=int] [loop_count=int] [inter_thread_num=int] "
                "[mean_value=str] [std_value=str] [dyn_shape=str]\n",
                argv[0]);
        return -1;
    }
    ...
    if (argc >= 11) {
        dyn_shape_str = argv[10];
    }

    std::cout << "run concurrency demo [" << net_name << "], num_threads=" << num_threads
              << ", inter_thread_num=" << inter_thread_num << std::endl;

    if (inter_thread_num <= 1) {
        std::cout << "inter_thread_num <= 1, skip multi executor concurrency demo." << std::endl;
        return 0;
    }
    ...
    if (status != nullptr) {
        std::cerr << "SessionOptionsSpaceMITEnvInit failed." << std::endl;
        return -1;
    }
    ...
    auto create_input_values = [&](int thread_index) {
        std::vector<Ort::Value> values;
        values.reserve(input_count);
        for (size_t i = 0; i < input_count; i++) {
            values.push_back(session.CreatorInputValue(i));
        }

        if (input_count == 1 && values[0].GetTensorTypeAndShapeInfo().GetElementType() ==
                                    ONNXTensorElementDataType::ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT) {
            if (img_file_path != nullptr && strcmp(img_file_path, "None") != 0) {
                auto input_shape = session.GetInputShape(0);
                if (input_shape.size() == 4) {
                    float * input_img_data = values[0].GetTensorMutableData<float>();
            ...
            auto * input_data = values[0].GetTensorMutableData<float>();
            if (input_data != nullptr) {
                input_data[0] += static_cast<float>(thread_index);
            }
        }
    ...
    if (input_count == 1 && img_file_path != nullptr && strcmp(img_file_path, "None") != 0) {
        std::cout << "Read img file from " << img_file_path << std::endl;
    }

    auto warmup_input_values = create_input_values(0);
    time_start               = std::chrono::high_resolution_clock::now();
    auto output_tensors      = session.Run(warmup_input_values.data());
    ...
    auto thread_input_values0 = create_input_values(0);
    auto thread_input_values1 = create_input_values(0);
    std::cout << "multi executor enabled, inter_thread_num=" << inter_thread_num
              << ", affinity_a=<auto>, affinity_b=<auto>" << std::endl;

    {
        std::vector<Ort::Value> warmup_outputs0;
        std::vector<Ort::Value> warmup_outputs1;
        double                  warmup_thread0_cost_ms = 0.0;
        double                  warmup_thread1_cost_ms = 0.0;
        std::thread             warmup_worker0([&]() {
            auto start             = std::chrono::high_resolution_clock::now();
            warmup_outputs0        = session.Run(thread_input_values0.data());
            warmup_thread0_cost_ms = std::chrono::duration_cast<std::chrono::microseconds>(
        ...
        std::thread             warmup_worker1([&]() {
            auto start             = std::chrono::high_resolution_clock::now();
            warmup_outputs1        = session.Run(thread_input_values1.data());
            warmup_thread1_cost_ms = std::chrono::duration_cast<std::chrono::microseconds>(
                                         std::chrono::high_resolution_clock::now() - start)
                                         .count() /
                                     1000.0;
        });
        warmup_worker0.join();
        warmup_worker1.join();
        std::cout << "multi executor warmup finished: thread0=" << warmup_thread0_cost_ms
                  << " ms, thread1=" << warmup_thread1_cost_ms
                  << " ms, max=" << std::max(warmup_thread0_cost_ms, warmup_thread1_cost_ms) << " ms." << std::endl;
    }

    std::vector<float> time_duration_vec;
    for (size_t i = 0; i < loop_count; i++) {
        std::vector<Ort::Value> thread_outputs0;
        std::vector<Ort::Value> thread_outputs1;
        double                  thread0_cost_ms = 0.0;
        double                  thread1_cost_ms = 0.0;

        std::thread worker0([&]() {
            auto start      = std::chrono::high_resolution_clock::now();
            thread_outputs0 = session.Run(thread_input_values0.data());
            thread0_cost_ms = std::chrono::duration_cast<std::chrono::microseconds>(
                                  std::chrono::high_resolution_clock::now() - start)
                                  .count() /
                              1000.0;
        });
        ...
        std::thread worker1([&]() {
            auto start      = std::chrono::high_resolution_clock::now();
            thread_outputs1 = session.Run(thread_input_values1.data());
            thread1_cost_ms = std::chrono::duration_cast<std::chrono::microseconds>(
                                  std::chrono::high_resolution_clock::now() - start)
                                  .count() /
                              1000.0;
        });
        ...
        time_duration_vec.push_back(std::max(thread0_cost_ms, thread1_cost_ms));

        if (i % 10 == 0) {
            std::cout << "concurrent inference time cost: thread0=" << thread0_cost_ms
                      << " ms, thread1=" << thread1_cost_ms << " ms. outputs saved for loop " << i << "."
                      << std::endl;
        }
    }
