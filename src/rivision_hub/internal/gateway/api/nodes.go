...
import (
	...
	"github.com/rivision/rivision-hub/internal/gateway/auth"
...
type NodeRegisterRequest struct {
	...
	AgentPort         int      `json:"agent_port"` // Node Agent 端口 (默认 9181)
...
func (h *Handlers) ListNodesSnapshot(c *gin.Context) {
	...
	nodeData := make([]map[string]interface{}, len(nodeList))
...
// GetCapabilities 获取集群能力汇总
// GET /api/v1/nodes/capabilities
func (h *Handlers) GetCapabilities(c *gin.Context) {
	registry := h.client.GetRegistry()
	summary := registry.GetCapabilitySummary()

	// 获取各类型节点详情
	yoloNodes := registry.GetNodesWithCapability(nodes.CapabilityYOLO)
	vlmNodes := registry.GetNodesWithCapability(nodes.CapabilityVLM)
	clipNodes := registry.GetNodesWithCapability(nodes.CapabilityCLIP)

	yoloList := make([]string, len(yoloNodes))
	for i, n := range yoloNodes {
		yoloList[i] = n.ID
	}

	vlmList := make([]string, len(vlmNodes))
	for i, n := range vlmNodes {
		vlmList[i] = n.ID
	}

	clipList := make([]string, len(clipNodes))
...
// GetNodeCapabilities 获取单个节点的能力信息
// GET /api/v1/nodes/:id/capabilities
func (h *Handlers) GetNodeCapabilities(c *gin.Context) {
	nodeID := c.Param("id")
	...
	caps := nodes.GetNodeCapabilities(node)
...
// GetNodesByCapability 按能力筛选节点
// GET /api/v1/nodes/by-capability/:capability
func (h *Handlers) GetNodesByCapability(c *gin.Context) {
	capStr := c.Param("capability")
	cap := nodes.WorkerCapability(capStr)

	// 验证能力类型
	validCaps := map[nodes.WorkerCapability]bool{
		nodes.CapabilityYOLO:   true,
		nodes.CapabilityVLM:    true,
		nodes.CapabilityCLIP:   true,
		nodes.CapabilityASR:    true,
		nodes.CapabilityAction: true,
	}

	if !validCaps[cap] {
		c.JSON(http.StatusBadRequest, gin.H{
			"success": false,
			"error":   fmt.Sprintf("无效的能力类型: %s", capStr),
			"valid_capabilities": []string{"yolo", "vlm", "clip", "asr", "action"},
		})
		return
	}

	registry := h.client.GetRegistry()
	nodeList := registry.GetNodesWithCapability(cap)

	nodeData := make([]map[string]interface{}, len(nodeList))
	for i, node := range nodeList {
		nodeData[i] = node.ToDict()
	}
...
func (h *Handlers) SelectNodeForTask(c *gin.Context) {
	var req struct {
	...
	registry := h.client.GetRegistry()
	selectedNode := registry.SelectNodeForTask(req.TaskType)

	if selectedNode == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{
			"success": false,
			"error":   fmt.Sprintf("没有满足任务 '%s' 要求的可用节点", req.TaskType),
		})
		return
	}
