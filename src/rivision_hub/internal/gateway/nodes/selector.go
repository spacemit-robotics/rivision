...
// ★ 默认用于VLM任务，使用 GetAvailableNodesForCapability 指定其他能力
func (ns *NodeSelector) GetAvailableNodes(exclude map[string]bool) []*Node {
	return ns.GetAvailableNodesForCapability(exclude, CapabilityVLM)
}

// GetAvailableNodesForCapability 根据能力类型获取可用节点
func (ns *NodeSelector) GetAvailableNodesForCapability(exclude map[string]bool, cap WorkerCapability) []*Node {
	...
	switch cap {
	case CapabilityYOLO:
		candidates = ns.registry.GetHealthyWithYolo()
	case CapabilityVLM:
		candidates = ns.registry.GetHealthyWithLlama()
	default:
		candidates = ns.registry.GetHealthy()
	}

	available := make([]*Node, 0, len(candidates))
