...
type Node struct {
	...
	Type         string                 `json:"type" yaml:"type"`                 // worker, gateway, etc.
	...
	Capabilities map[string]interface{} `json:"capabilities" yaml:"capabilities"` // 能力信息
	...
	AgentPort        int                `json:"agent_port"`        // Node Agent 端口 (9181)
	YOLOPort         int                `json:"yolo_port"`         // YOLO Server 端口 (9081)
	OwlURL           string             `json:"owl_url"`           // Worker OWL API URL (e.g. http://192.168.1.10:15123)
	ZLMHost          string             `json:"zlm_host"`          // Worker ZLM IP for receiving RTP (SIP-media split)
	ZLMHTTPPort      int                `json:"zlm_http_port"`     // Worker ZLM HTTP API port (default 80)
	ZLMRTSPPort      int                `json:"zlm_rtsp_port"`     // Worker ZLM RTSP port (default 554)
	...
	YoloModel        string             `json:"yolo_model"`         // YOLO 模型名
	VlmModel         string             `json:"vlm_model"`          // VLM 模型名
	EmbedEnabled     bool               `json:"embed_enabled"`      // Embed 服务启用
	EmbedHealthy     bool               `json:"embed_healthy"`      // Embed 服务健康
	EmbedModel       string             `json:"embed_model"`        // Embed 模型名
...
type HeartbeatData struct {
	...
	Load          float64                `json:"load"`
	CPUPercent    float64                `json:"cpu_percent"`              // Worker 直接上报
	MemPercent    float64                `json:"mem_percent"`              // Worker 直接上报
	DiskPercent   float64                `json:"disk_percent"`             // Worker 直接上报
	Load1         float64                `json:"load_1"`                   // Worker 直接上报
	ActiveStreams int                    `json:"active_streams"`           // Worker 直接上报
	LlamaStatus   map[string]interface{} `json:"llama_status"`             // 兼容旧版
	VLMStatus     map[string]interface{} `json:"vlm_status"`               // 新版 VLM 状态
	YoloStatus    map[string]interface{} `json:"yolo_status"`
	EmbedStatus   map[string]interface{} `json:"embed_status"`             // Embed 服务状态
	SystemStats   map[string]interface{} `json:"system_stats"`
	Resources     map[string]interface{} `json:"resources"`
	GpuStats      map[string]interface{} `json:"gpu_stats"`
	Capabilities  map[string]interface{} `json:"capabilities"`             // 能力/模型信息
	Version       string                 `json:"version"`                  // Worker 版本
	UptimeSeconds int64                  `json:"uptime_seconds"`           // 运行时间(秒)
...
func (n *Node) UpdateStatus(data *HeartbeatData) {
	...
	// 支持 VLM 状态 (新版 Worker 使用 vlm_status)
	vlmStatus := data.LlamaStatus
	if data.VLMStatus != nil {
		vlmStatus = data.VLMStatus
	}
	if vlmStatus != nil {
		if v, ok := vlmStatus["healthy"]; ok {
			if b, ok := v.(bool); ok {
				n.LlamaHealthy = b
			}
		}
		// ★ 解析 VLM 服务启用状态
		if v, ok := vlmStatus["enabled"]; ok {
			if b, ok := v.(bool); ok {
				n.LlamaEnabled = b
			}
		}
		if v, ok := vlmStatus["slots_idle"]; ok {
			n.SlotsIdle = toInt(v)
		}
		if v, ok := vlmStatus["slots_processing"]; ok {
			n.SlotsProc = toInt(v)
		}
	}
	...
	if data.YoloStatus != nil {
		...
		// ★ 解析 yolo 模型名
		if v, ok := data.YoloStatus["model"]; ok {
	...
	// ★ 解析 Embed 状态
	if data.EmbedStatus != nil {
		if v, ok := data.EmbedStatus["enabled"]; ok {
			if b, ok := v.(bool); ok {
				n.EmbedEnabled = b
			}
		}
		if v, ok := data.EmbedStatus["healthy"]; ok {
			if b, ok := v.(bool); ok {
				n.EmbedHealthy = b
			}
		}
		if v, ok := data.EmbedStatus["model"]; ok {
			if s, ok := v.(string); ok && s != "" {
				n.EmbedModel = s
			}
		}
	...
	if data.UptimeSeconds > 0 {
		n.UptimeSeconds = data.UptimeSeconds
	}

	// ★ 解析 Capabilities (更新模型信息)
	if data.Capabilities != nil {
		n.Capabilities = data.Capabilities
	}

	// Worker 直接上报的顶层字段
	if data.CPUPercent > 0 {
		n.CPUPercent = data.CPUPercent
	}
	if data.MemPercent > 0 {
		n.MemoryPercent = data.MemPercent
	}
	...
	if data.Resources != nil {
		...
		// 从 Resources 提取 CPU/Memory/GPU
		if v := toFloat64(data.Resources["cpu_percent"]); v > 0 {
			n.CPUPercent = v
		}
		if v := toFloat64(data.Resources["memory_percent"]); v > 0 {
			n.MemoryPercent = v
		}
		if v := toFloat64(data.Resources["gpu_memory_percent"]); v > 0 {
			n.GpuMemoryPercent = v
		}
	}
}
...
func (n *Node) ToDict() map[string]interface{} {
	...
	return map[string]interface{}{
		...
		"yolo_model":       n.YoloModel,       // ★ YOLO 模型名
		"vlm_model":        n.VlmModel,        // ★ VLM 模型名
		"embed_enabled":    n.EmbedEnabled,    // ★ Embed 服务启用
		"embed_healthy":    n.EmbedHealthy,    // ★ Embed 服务健康
		"embed_model":      n.EmbedModel,      // ★ Embed 模型名
		"version":          n.Version,         // ★ Worker 版本
		"uptime_seconds":   n.UptimeSeconds,   // ★ 运行时间
