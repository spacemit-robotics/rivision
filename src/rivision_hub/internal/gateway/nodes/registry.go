...
type Registry struct {
	...
	persistPath string // 如果非空，注册/注销时自动持久化到此文件
}
...
// SetPersistPath 设置持久化路径，注册/注销时自动保存
...
// persist 将当前节点列表保存到 YAML 文件（如果设置了 persistPath）
func (r *Registry) persist() {
	...
	r.mu.RLock()
	config := NodesConfig{
		Cluster: ClusterConfig{Name: r.clusterName},
	}
	...
	if err != nil {
		log.Printf("[registry] persist marshal error: %v", err)
		return
	}
	if err := os.WriteFile(r.persistPath, data, 0644); err != nil {
		log.Printf("[registry] persist write error: %v", err)
	}
}
