<template>
  <div class="video-grid" :class="`grid-${layout}`">
    <VideoCell
      ...
      :is-yolo-enabled="cameraId ? yoloEnabledCameras?.has(cameraId) : false"
      ...
      @toggle-yolo="(camId: string, enabled: boolean) => emit('toggle-yolo', camId, enabled)"
      @toggle-vlm="(camId: string, enabled: boolean) => emit('toggle-vlm', camId, enabled)"
    />
          ...
          <div class="camera-options">
            ...
            >
              ...
              <div class="camera-info">
                ...
                <div class="camera-status" :class="{ online: camera.status === 'online' }">
...
const emit = defineEmits<{
  ...
  (e: 'update:layout', layout: GridLayout): void
...
const availableCameras = computed((): Camera[] => {
  return props.cameras.filter((camera: Camera) => 
    !displayCameras.value.includes(camera.id)
  )
})
